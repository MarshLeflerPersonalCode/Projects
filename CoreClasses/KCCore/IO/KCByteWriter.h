...
{
public:
	KCByteWriter(uint32 iGrowBy = 1000)
	{
		m_ByteArray.setGrowBy(iGrowBy);				
	...
	FORCEINLINE bool			seekEnd() { m_iCurrentByteIndex = m_ByteArray.getCount(); }
	FORCEINLINE bool			seek(uint32 iLocation)
	{
		if (iLocation >= 0 && iLocation < m_ByteArray.getCount())
		{
	...
	{
		m_ByteArray.reserve(iMemoryReserve);
	}

	//returns the memory
	const uint8 *				getByteArray() const { return m_ByteArray.getMemory(); }
	//returns the memory as a char array
	const char *				getByteArrayAsChar() const { return (char * )m_ByteArray.getMemory(); }
	//returns the memory size
	uint32						getByteArrayCount(){ return m_ByteArray.getCount(); }
	...
	FORCEINLINE bool			writeValue(const void *pValue, size_t iSize)
	{
		...
		for (iIndex; iIndex < iFinalIndex; iIndex++)
		{
			...
			iArrayIndex++;
			if (iIndex >= m_ByteArray.getCount())
			{
				m_ByteArray.add(iValue);
			}
			else
			{
				m_ByteArray[iIndex] = iValue;
			}
		}
