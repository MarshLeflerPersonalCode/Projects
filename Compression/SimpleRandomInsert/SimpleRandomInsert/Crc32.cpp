...
// Static CRC table
uint32 CCrc32Static::s_arrdwCrc32Table[256] =
{
...
#define MAKEQWORD(a, b)	\
	((int64)( ((int64) ((uint32) (a))) << 32 | ((uint32) (b))))
...
//***********************************************
inline void CCrc32Static::CalcCrc32(const BYTE byte, uint32 &dwCrc32)
{
...
uint32 CCrc32Static::getHash32( const void *key, int len, uint32 seed )
{
...
//***********************************************
/*
bool CCrc32Static::GetFileSizeQW(const HANDLE hFile, int64 &qwSize)
{
...
*/
uint32 CCrc32Static::createChecksumFromBytes( const byte *pBytes, DWORD iBytes, uint32 &dwCrc32)
{
	dwCrc32 = 0xFFFFFFFF;
	BYTE buffer[4096];
	DWORD dwBytesRead;
	...
	{
		{
			// There is a bug i;n the Microsoft compilers where inline assembly
			...
			while( totalBytesRead < iBytes )
			{
				dwBytesRead = iBytes - totalBytesRead;
				if( dwBytesRead > 4096 )
				{
					dwBytesRead = 4096;
				}
				memcpy( &buffer, &pBytes[ totalBytesRead ], dwBytesRead );
...
/*
uint32 CCrc32Static::createChecksumFromFile(LPCTSTR szFilename, uint32 &dwCrc32)
{
	_ASSERTE(szFilename);
	_ASSERTE(lstrlen(szFilename));

	uint32 dwErrorCode = NO_ERROR;
	HANDLE hFile = NULL;
