...
namespace Library.UnitType
{
	...
	{
        private static UnitTypeManager g_UnitTypeManager = null;
        ...
        private Dictionary<string, List<string>> m_UnitTypeLookNames = new Dictionary<string, List<string>>();
		public UnitTypeManager()
		{
			m_UnitTypeManagerConfig = UnitTypeManagerConfig.createFromConfigFile();
            g_UnitTypeManager = this;
        ...
        public static UnitTypeManager getUnitTypeManager() { return g_UnitTypeManager; }

		public bool needsToSave { get; set; }

        public void resetUnitTypeLookupNames()
        {
            m_UnitTypeLookNames.Clear();
        }
        ...
        public List<string> getUnitTypeNames(string strCategory, string strUnitTypeFilter)
        {
            
            string strCombinedName = strCategory + "_" + strUnitTypeFilter;
            if (m_UnitTypeLookNames.ContainsKey(strCombinedName))
            {
                return m_UnitTypeLookNames[strCombinedName];
            }
            UnitTypeCategory mCategory = getCategory(strCategory);
            if (mCategory != null)
            {
                List<UnitType> mList = null;
                if (strUnitTypeFilter != "")
                {
                    mList = mCategory.getUnitTypesOfType(strUnitTypeFilter);
                }
                else
                {
                    mList = mCategory.getUnitTypes();
                }
                List<string> mNewList = new List<string>();
                foreach(UnitType mType in mList)
                {
                    mNewList.Add(mType.unitTypeName);
                }
                m_UnitTypeLookNames[strCombinedName] = mNewList;
                return mNewList;
            }
            return null;
        }
